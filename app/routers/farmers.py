...
async def list_farmers(
    ...
    if county:
        where_clauses.append("loc.county = $county")
...
async def get_credit_profile(farmer_id: str):
    ...
    async with get_session() as session:
        ...
        record = await result.single()
    if not record:
        raise HTTPException(status_code=404, detail=f"No credit profile for farmer '{farmer_id}'")

    data = dict(record)
    # Filter out null-keyed items that Neo4j returns when optional matches miss
    ...
    if data.get("modelRun") and not data["modelRun"].get("runId"):
        data["modelRun"] = None
    return data
...
async def get_financial_profile(farmer_id: str):
    ...
    async with get_session() as session:
        ...
        record = await result.single()
    if not record:
        raise HTTPException(status_code=404, detail=f"No financial profile for farmer '{farmer_id}'")
    return dict(record)
...
async def get_farmer_expenses(
    ...
    where = "WHERE f.farmerId = $farmerId"
    params: dict = {"farmerId": farmer_id}
    if category:
        where += " AND e.category = $category"
        params["category"] = category

    cypher = f"""
        MATCH (f:Farmer)-[:INCURRED]->(e:ExpenseTransaction)
        {where}
        RETURN
